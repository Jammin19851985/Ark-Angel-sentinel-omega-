...
const AvatarOrb: React.FC = () => {
  ...
  // Eye Tracking Logic: Calculates vector from center of screen to mouse
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      // Normalize coordinates -1 to 1 based on window size
      // Dampening factor (dividing by larger numbers) controls sensitivity
  ...
  // Dynamic Color Palette based on System State
  const getColors = () => {
      if (killSwitchActive) {
          return { primary: '#ef4444', secondary: '#7f1d1d', glow: '#b91c1c', eye: '#ff0000' }; // Red/Crisis
      }
      if (isGodMode) {
          return { primary: '#fbbf24', secondary: '#78350f', glow: '#d97706', eye: '#fffbeb' }; // Gold/God
      }
      return { primary: '#06b6d4', secondary: '#164e63', glow: '#0891b2', eye: '#22d3ee' }; // Cyan/Default
  };
  ...
  return (
    <div className="w-[400px] h-[400px] pointer-events-none select-none relative z-0">
      <motion.div
        ...
        animate={{ y: [0, -10, 0] }} // Levitation breathing effect
      ...
      >
        <svg width="100%" height="100%" viewBox="0 0 400 400" className="overflow-visible drop-shadow-[0_0_30px_rgba(0,0,0,0.8)]">
          ...
          {/* --- LAYER 3: MAIN HEAD SHELL (Geometric Cyber-Skull) --- */}
          <path 
          ...
          {/* Pupils Group - Tracks Mouse */}
          <motion.g 
            animate={{ 
                x: mousePos.x * 12, // Movement range
                y: mousePos.y * 8 
            }}
          ...
          {/* --- LAYER 5: MOUTH / VOICE BOX --- */}
          <path d="M 170 260 L 230 260 L 225 280 L 175 280 Z" fill="#000" stroke={colors.secondary} strokeWidth="1" />
          
          {/* Dynamic Voice Activity */}
          {(systemStatus === 'EXECUTING' || systemStatus === 'RUNNING') && (
             <motion.g>
                {/* Sound waves emitted from mouth */}
                <motion.path 
        ...
        <div className="absolute -bottom-10 flex flex-col items-center">
            ...
            <div className={`px-4 py-1 rounded-sm border backdrop-blur-md transition-colors duration-500 ${killSwitchActive ? 'bg-red-950/50 border-red-500 text-red-400' : 'bg-black/60 border-cyan-500/30 text-cyan-400'}`}>
                <span className="text-[10px] font-mono tracking-[0.3em] font-bold uppercase shadow-glow">
                    {isGodMode ? 'ARCHANGEL_Ω' : 'SENTINEL_MK1'}
                </span>
            </div>
        </div>
      </motion.div>
