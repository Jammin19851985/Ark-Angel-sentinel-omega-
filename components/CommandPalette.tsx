...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose, setActiveView }) => {
    ...
    const commands: Command[] = useMemo(() => [
        ...
        // Fix: Removed non-existent 'activeView' from dependency array
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center px-3 py-3 rounded-lg transition-all text-left group relative overflow-hidden ${
                                            isSelected 
                                            ? 'bg-cyan-900/20 border border-cyan-500/50 shadow-[inset_0_0_10px_rgba(6,182,212,0.1)]' 
                                    ...
                                    >
                                        ...
                                        <div className={`p-2 rounded-md mr-4 transition-colors ${isSelected ? 'bg-cyan-950/50 text-cyan-400' : 'bg-slate-900 text-slate-500'}`}>
                                            {cmd.icon}
                                        </div>

                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                <span className={`text-sm font-bold tracking-widest uppercase ${isSelected ? 'text-white' : 'text-slate-400'}`}>
                                                    {cmd.label}
                                                </span>
                                            ...
                                            <div className={`text-[10px] mt-0.5 truncate ${isSelected ? 'text-cyan-200/60' : 'text-slate-600'}`}>
                                                {cmd.description}
                                            </div>
                                        </div>

                                        {isSelected && (
                                            <div className="ml-4 text-[10px] text-cyan-500 animate-pulse font-bold">
                                                EXECUTE &gt;&gt;
                                            </div>
                                        )}
                                    </button>
