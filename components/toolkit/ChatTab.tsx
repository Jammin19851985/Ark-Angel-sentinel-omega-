...
const ChatTab: React.FC = () => {
    ...
    const Toggle: React.FC<{ enabled: boolean; onToggle: () => void; label: string; icon: React.ReactNode }> = ({ enabled, onToggle, label, icon }) => (
        <button
            ...
            onClick={onToggle}
            className="flex items-center space-x-2 cursor-pointer rounded-md p-1 -m-1 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 focus:ring-offset-slate-800"
        >
            <div className={`relative inline-flex h-5 w-9 flex-shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${enabled ? 'bg-amber-600' : 'bg-black/50 backdrop-blur-sm border-slate-700'}`}>
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex items-center space-x-4 mb-4 p-3 bg-black/50 backdrop-blur-sm rounded-lg">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-black/30 backdrop-blur-sm rounded-lg border border-slate-800 space-y-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex flex-col ${msg.author === 'user' ? 'items-end' : 'items-start'}`}>
                        <div className={`rounded-lg px-4 py-2 max-w-xl ${msg.author === 'user' ? 'bg-amber-900/50 backdrop-blur-sm' : 'bg-black/50 backdrop-blur-sm'}`}>
